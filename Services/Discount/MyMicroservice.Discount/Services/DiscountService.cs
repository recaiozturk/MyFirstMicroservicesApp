...
namespace MyMicroservice.Discount.Services
{
    public class DiscountService : IDiscountService
    {
        ...
        {
            var discont = (await _dbConnection.QueryAsync<Models.Discount>("select * from discount where id=@Id", new { Id = id })).SingleOrDefault();

            if(discont==null)
                return Response<Models.Discount>.Fail("Discount not found",404);

            return Response<Models.Discount>.Success(discont, 200);
        }
        ...
        public async Task<Response<NoContent>> Save(Models.Discount discount)
        {
            ...
            return Response<NoContent>.Fail("an error occured while added", 500);
        }
